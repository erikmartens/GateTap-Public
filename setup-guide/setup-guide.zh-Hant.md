...
門禁控制器是一種管理門、閘門、車庫或柵欄開啟的裝置 — 例如啟動門鈴或閘門馬達。
它通常從以下來源接收開啟訊號：
...
許多現代門禁系統都連接到本機網路，並可透過瀏覽器中的 Web 介面操作。GateTap 會直接連接到你的門禁系統，讓你可以從裝置上方便地操作它。


## 開始之前

請確保你的裝置與門禁控制器連接到同一個本機網路。例如，確認 iPhone 連接的是家中的 Wi‑Fi，而不是行動數據連線。
...
有些門禁控制器仍使用預設登入憑證。常見範例是使用者名稱 `abc`，密碼 `654321`。

其他常見的預設使用者名稱包括 `user`、`admin` 或 `123`。你可以搭配常見密碼如 `1234`、`user` 或 `password`，或其變體一起嘗試。
...
開啟 GateTap。如果新增控制器的頁面沒有自動出現，請切換到 "Controller" 分頁，然後點一下右上角導覽列中的 "+" 按鈕。
...
GateTap 也包含示範模式。你可以從 app 內啟動虛擬門禁控制器，它會像真實系統一樣提供管理介面。之後你可以使用顯示的 IP 位址和登入憑證，像一般控制器一樣新增它。

這提供一個已知可運作的測試路徑，用來探索 GateTap 的功能，即使你目前沒有實體門禁控制器。
