<!--
Generated from GateTap app legal JSON.
Do not edit manually.
Version: 1.1
Last updated: 2026-04-27
-->

# Användarvillkor

---

🌍 **This Document is available in other Languages:**  
[🇺🇸 English](../en-US/terms-of-use.md) | [🇩🇪 Deutsch](../de-DE/terms-of-use.de.md) | [🇸🇦 العربية](../ar/terms-of-use.ar.md) | [🇪🇸 Català](../ca/terms-of-use.ca.md) | [🇨🇿 Čeština](../cs/terms-of-use.cs.md) | [🇩🇰 Dansk](../da/terms-of-use.da.md) | [🇬🇷 Ελληνικά](../el/terms-of-use.el.md) | [🇪🇸 Español](../es/terms-of-use.es.md) | [🇲🇽 Español (México)](../es-MX/terms-of-use.es-MX.md) | [🇫🇮 Suomi](../fi/terms-of-use.fi.md) | [🇫🇷 Français](../fr/terms-of-use.fr.md) | [🇮🇱 עברית](../he/terms-of-use.he.md) | [🇮🇳 हिन्दी](../hi/terms-of-use.hi.md) | [🇭🇷 Hrvatski](../hr/terms-of-use.hr.md) | [🇭🇺 Magyar](../hu/terms-of-use.hu.md) | [🇮🇩 Bahasa Indonesia](../id/terms-of-use.id.md) | [🇮🇹 Italiano](../it/terms-of-use.it.md) | [🇯🇵 日本語](../ja/terms-of-use.ja.md) | [🇰🇷 한국어](../ko/terms-of-use.ko.md) | [🇲🇾 Bahasa Melayu](../ms/terms-of-use.ms.md) | [🇳🇴 Norsk Bokmål](../nb/terms-of-use.nb.md) | [🇳🇱 Nederlands](../nl/terms-of-use.nl.md) | [🇵🇱 Polski](../pl/terms-of-use.pl.md) | [🇧🇷 Português (Brasil)](../pt-BR/terms-of-use.pt-BR.md) | [🇵🇹 Português (Portugal)](../pt-PT/terms-of-use.pt-PT.md) | [🇷🇴 Română](../ro/terms-of-use.ro.md) | [🇷🇺 Русский](../ru/terms-of-use.ru.md) | [🇸🇰 Slovenčina](../sk/terms-of-use.sk.md) | 🇸🇪 Svenska | [🇹🇭 ไทย](../th/terms-of-use.th.md) | [🇹🇷 Türkçe](../tr/terms-of-use.tr.md) | [🇺🇦 Українська](../uk/terms-of-use.uk.md) | [🇻🇳 Tiếng Việt](../vi/terms-of-use.vi.md) | [🇨🇳 简体中文](../zh-Hans/terms-of-use.zh-Hans.md) | [🇹🇼 繁體中文](../zh-Hant/terms-of-use.zh-Hant.md)

---



**Version:** 1.1  
**Senast uppdaterad:** 2026-04-27

## 1. Översikt
GateTap ("appen") tillhandahåller ett mobilt gränssnitt för att interagera med lokalt värdade åtkomstkontrollsystem via ditt lokala nätverk.

## 2. Ingen anslutning
GateTap är en oberoende applikation och är inte ansluten till, godkänd av eller ansluten till någon hårdvarutillverkare eller leverantör av åtkomstkontroll.

## 3. Användaransvar
Genom att använda denna app godkänner du att:
- Du har behörighet att komma åt och kontrollera den anslutna enheten
- Du är ansvarig för alla åtgärder som utförs genom appen
- Du kommer inte att använda appen för obehörig åtkomst eller olagliga ändamål

## 4. Säkerhet
GateTap lagrar autentiseringsuppgifter lokalt på din enhet. Du är ansvarig för att säkra din enhet (t.ex. lösenord, Face ID).

Appen kommunicerar direkt med enheter på ditt lokala nätverk och tillhandahåller ingen infrastruktur för fjärråtkomst.

## 5. Ingen garanti
Appen tillhandahålls "i befintligt skick" utan garantier av något slag. Vi garanterar inte:
- Kompatibilitet med alla enheter
- Kontinuerlig tillgänglighet
- Felfri drift

## 6. Ansvarsbegränsning
Byggherren ansvarar inte för skador som uppstår på grund av:
- Obehörig tillgång till egendom
- Felkonfiguration av enheter
- Nätverksrelaterade frågor
- Användning av appen i kritiska eller säkerhetskänsliga miljöer

## 7. Ändringar
Dessa villkor kan komma att uppdateras då och då. Fortsatt användning av appen innebär att du accepterar de uppdaterade villkoren.

## 8. Kontakt
För frågor, kontakta: [erikmartens.developer@gmail.com](mailto:erikmartens.developer@gmail.com)

