<!--
Generated from GateTap app legal JSON.
Do not edit manually.
Version: 1.1
Last updated: 2026-04-27
-->

# Conditions d'utilisation

---

🌍 **This Document is available in other Languages:**  
[🇺🇸 English](../en-US/terms-of-use.md) | [🇩🇪 Deutsch](../de-DE/terms-of-use.de.md) | [🇸🇦 العربية](../ar/terms-of-use.ar.md) | [🇪🇸 Català](../ca/terms-of-use.ca.md) | [🇨🇿 Čeština](../cs/terms-of-use.cs.md) | [🇩🇰 Dansk](../da/terms-of-use.da.md) | [🇬🇷 Ελληνικά](../el/terms-of-use.el.md) | [🇪🇸 Español](../es/terms-of-use.es.md) | [🇲🇽 Español (México)](../es-MX/terms-of-use.es-MX.md) | [🇫🇮 Suomi](../fi/terms-of-use.fi.md) | 🇫🇷 Français | [🇮🇱 עברית](../he/terms-of-use.he.md) | [🇮🇳 हिन्दी](../hi/terms-of-use.hi.md) | [🇭🇷 Hrvatski](../hr/terms-of-use.hr.md) | [🇭🇺 Magyar](../hu/terms-of-use.hu.md) | [🇮🇩 Bahasa Indonesia](../id/terms-of-use.id.md) | [🇮🇹 Italiano](../it/terms-of-use.it.md) | [🇯🇵 日本語](../ja/terms-of-use.ja.md) | [🇰🇷 한국어](../ko/terms-of-use.ko.md) | [🇲🇾 Bahasa Melayu](../ms/terms-of-use.ms.md) | [🇳🇴 Norsk Bokmål](../nb/terms-of-use.nb.md) | [🇳🇱 Nederlands](../nl/terms-of-use.nl.md) | [🇵🇱 Polski](../pl/terms-of-use.pl.md) | [🇧🇷 Português (Brasil)](../pt-BR/terms-of-use.pt-BR.md) | [🇵🇹 Português (Portugal)](../pt-PT/terms-of-use.pt-PT.md) | [🇷🇴 Română](../ro/terms-of-use.ro.md) | [🇷🇺 Русский](../ru/terms-of-use.ru.md) | [🇸🇰 Slovenčina](../sk/terms-of-use.sk.md) | [🇸🇪 Svenska](../sv/terms-of-use.sv.md) | [🇹🇭 ไทย](../th/terms-of-use.th.md) | [🇹🇷 Türkçe](../tr/terms-of-use.tr.md) | [🇺🇦 Українська](../uk/terms-of-use.uk.md) | [🇻🇳 Tiếng Việt](../vi/terms-of-use.vi.md) | [🇨🇳 简体中文](../zh-Hans/terms-of-use.zh-Hans.md) | [🇹🇼 繁體中文](../zh-Hant/terms-of-use.zh-Hant.md)

---



**Version :** 1.1  
**Dernière mise à jour :** 2026-04-27

## 1. Aperçu
GateTap (« l'Application ») fournit une interface mobile pour interagir avec les systèmes de contrôle d'accès hébergés localement via votre réseau local.

## 2. Aucune affiliation
GateTap est une application indépendante et n'est affiliée, approuvée ou connectée à aucun fabricant de matériel ou fournisseur de contrôle d'accès.

## 3. Responsabilité de l'utilisateur
En utilisant cette application, vous acceptez ce qui suit :
- Vous avez la permission d'accéder et de contrôler l'appareil connecté
- Vous êtes responsable de toutes les actions effectuées via l'application
- Vous n'utiliserez pas l'application à des fins d'accès non autorisé ou à des fins illégales

## 4. Sécurité
GateTap stocke les informations d'identification localement sur votre appareil. Vous êtes responsable de la sécurisation de votre appareil (par exemple, mot de passe, Face ID).

L'application communique directement avec les appareils de votre réseau local et ne fournit aucune infrastructure d'accès à distance.

## 5. Aucune garantie
L'application est fournie « telle quelle » sans garantie d'aucune sorte. Nous ne garantissons pas :
- Compatibilité avec tous les appareils
- Disponibilité continue
- Fonctionnement sans erreur

## 6. Limitation de responsabilité
Le développeur ne sera pas responsable des dommages résultant de :
- Accès non autorisé à la propriété
- Mauvaise configuration des appareils
- Problèmes liés au réseau
- Utilisation de l'Application dans des environnements critiques ou sensibles en matière de sécurité

## 7. Modifications
Ces conditions peuvent être mises à jour de temps à autre. L'utilisation continue de l'application constitue l'acceptation des conditions mises à jour.

## 8. Contacter
Pour toute question, contactez : [erikmartens.developer@gmail.com](mailto:erikmartens.developer@gmail.com)

