...
## 1. 概述
GateTap（「本 App」）提供一個行動介面，用於透過你的本地網路與本機託管的門禁控制系統互動。

## 2. 無關聯關係
GateTap 是獨立應用程式，不隸屬於、未獲背書，也不與任何硬體製造商或門禁控制服務提供者存在關聯。
...
- 你有權存取並控制已連接的裝置
- 你對透過本 App 執行的所有操作負責
- 你不會將本 App 用於未經授權的存取或違法目的

## 4. 安全
GateTap 會將登入憑證本機儲存在你的裝置上。你有責任保護你的裝置，例如使用裝置密碼或 Face ID。

本 App 直接與本地網路中的裝置通訊，不提供任何遠端存取基礎設施。

## 5. 無擔保
本 App 依「現況」提供，不附帶任何形式的擔保。我們不保證：
