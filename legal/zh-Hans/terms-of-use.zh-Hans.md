...
## 1. 概述
GateTap（“本 App”）提供一个移动界面，用于通过你的本地网络与本地托管的门禁控制系统交互。

## 2. 无关联关系
GateTap 是独立应用，不隶属于、未获得背书，也不与任何硬件制造商或门禁控制服务提供商存在关联。
...
- 你有权访问并控制已连接的设备
- 你对通过本 App 执行的所有操作负责
- 你不会将本 App 用于未经授权的访问或违法目的

## 4. 安全
GateTap 会将登录凭据本地保存在你的设备上。你有责任保护你的设备，例如使用设备密码或 Face ID。

本 App 直接与本地网络中的设备通信，不提供任何远程访问基础设施。

## 5. 无担保
本 App 按“现状”提供，不附带任何形式的担保。我们不保证：
